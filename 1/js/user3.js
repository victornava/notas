 ...
 * • put all Notas Code in one Class
 * • make all sizes relative to screen size
 * • improve event catching: http://www.artzstudio.com/2009/04/jquery-performance-rules/#leverage-event-delegation
 * • preload notas
 * • remove "px" from notas values when saving
 */
var changedNotas = [];
var notasList = {};
var activeNota = "none";
...
$.ajaxSetup({ //reference > http://docs.jquery.com/Ajax/jQuery.ajax#options
    ...
    beforeSend: function(d){
        $("#ajaxIcon").fadeIn("normal");
    },
    complete: function(){
        $("#ajaxIcon").fadeOut("normal");
    }
});
...
function createNota(){
    var n = NOTAS.prefs;
    n.pos_z = getTopZ() + 1 //note: this is dangerous
    var nota = drawNota(n);
    saveNotas([nota.htmlID]);
    delete n.pos_z; //note: this is dangerous
    return nota;
}
...
function Nota(args, container){ //Nota(args, container)
    ...
    var notaHtml = '<div id=' + this.htmlID + ' ><div class="notaCloseIcon">x</div><textarea class="notaTxa" wrap="soft"></textarea></div>';
    ...
    //notaDiv.mouseup(function(){ //TODO: fix frozen cursor on safari/chrome when click on notaDiv
...
Nota.prototype.setWidth = function(width){
    $("#" + this.htmlID).css("width", width);
    $("#" + this.htmlID).trigger("change");
}

Nota.prototype.setHeight = function(height){
    $("#" + this.htmlID).css("height", height);
    $("#" + this.htmlID).trigger("change");
}
