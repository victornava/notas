function ActionBar(id, container, items){
	...
	//todo: if id already exist, don't create another div just add stuff
...
ActionBar.prototype.addItem = function(name, html){
    var actionBar, span, ul;
    actionBar = $("#" + this.id);
    span = actionBar.append("<span></span>").children("span:last");
    span.addClass('actionBarItem');
    html ? span.html(html) : span.text(name);
    span.append("<ul class='actionBarItemList'></ul>");
    span.attr("id", "_" + name);
    
    var left = span.position().left - 1;
    //var top = span.position().top + span.height();
